...
PORT="${2:-${STEAMDB_CDP_PORT:-55685}}"
URL="${STEAMDB_BOOTSTRAP_URL:-https://steamdb.info/app/4165870/}"
...
if [ -z "$existing_pid" ]; then
  ...
  echo "SteamDB bootstrap browser already running on port $PORT (pid $existing_pid)."
fi

cat > "$ENV_FILE" <<EOF
export STEAMDB_PLAYWRIGHT_FALLBACK=1
export STEAMDB_PROFILE_DIR="$PROFILE_DIR"
export STEAMDB_CDP_ENDPOINT="http://127.0.0.1:$PORT"
export STEAMDB_BROWSER_CHANNEL="chrome"
export STEAMDB_REFERER="https://steamdb.info/app/4165870/"
EOF

printf '%s\n' "SteamDB browser bootstrap ready."
printf '%s\n' "Profile: $PROFILE_DIR"
printf '%s\n' "CDP endpoint: http://127.0.0.1:$PORT"
printf '%s\n' "Env file: $ENV_FILE"
printf '%s\n' ""
printf '%s\n' "Next steps:"
printf '%s\n' "1. If SteamDB needs manual interaction, switch to the dedicated Chrome window when convenient."
printf '%s\n' "2. Leave that dedicated browser running in the background."
printf '%s\n' "3. Source the env file before a run, or let check_steamdb.sh pick it up automatically."
