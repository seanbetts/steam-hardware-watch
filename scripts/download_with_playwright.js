...
async function main() {
  const [, , targetUrl, outputPath, metaPath] = process.argv;
  if (!targetUrl || !outputPath || !metaPath) {
    console.error("usage: download_with_playwright.js URL OUTPUT_PATH META_PATH");
    process.exit(1);
  }
  ...
  try {
    ...
    if (cdpEndpoint) {
      ...
      result = await page.evaluate(async ({ url, timeoutMs }) => {
      ...
      fs.writeFileSync(outputPath, Buffer.from(result.bodyBase64, "base64"));
    } else {
