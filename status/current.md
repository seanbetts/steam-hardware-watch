...
- `Same-time launch`: same-day availability for the full trio is ruled out. Steam Controller is public and purchasable; Machine and Frame still have no public package, package group, price, or exact availability. Steam Machine reservation package changenumbers have now moved three times: a multi-package cluster on `2026-05-12`, a follow-up bump across all four watched Machine packages on `2026-05-14`, and another all-four package bump on `2026-06-09`. The `2026-06-09` pass found no Valve public API readiness, and the two watched Steam Frame package IDs did not move. This makes Steam Machine the strongest current near-term signal, but it remains private backend movement rather than preorder confirmation. ImportGenius still shows relevant `GAME CONSOLE` shipments to `INGRAM MICRO C/O VALVE CORPORATION` through `2026-05-18`; this remains logistics corroboration, not public launch confirmation.

## Source Snapshot

### Komodo

- Komodo was reachable in the `2026-06-09` pass.
...
- No new Komodo section, media URL, asset exposure, or product timestamp movement appeared on `2026-06-09`. Treat the category addition as a real Komodo catalog-organization signal, not preorder confirmation.
...
- Current changenumbers from the `2026-06-09` run:
  - Controller app `4165870`: `35731145`
  - Frame app `4165890`: `35675573`
  - Machine app `4165910`: `35675573`
  - Controller package `1558609`: `35893035`
  - Machine package `1629446`: `36426730`
  - Machine package `1629447`: `36426723`
  - Machine package `1629458`: `36426735`
  - Machine package `1629460`: `36426739`
  - Frame packages `1629484`, `1629486`: `35672606`
- All four watched Steam Machine package changenumbers moved again on `2026-06-09`; watched Steam Frame package changenumbers did not move.
...
- The latest material Machine signal is the `2026-06-09` follow-up bump across all four watched Steam Machine package IDs, especially because the two watched Steam Frame package IDs did not move.
- Manual package-history context remains important: Steam Controller package `1558609` reportedly had a clustered change pattern plus one later follow-up change roughly two days before Valve's public purchase-timing update appeared. The Machine set now has the initial multi-package cluster and two later all-package follow-up bumps. Treat this as a stronger imminent-preorder heuristic, not confirmation.
...
- Raw snapshot: `/Users/sean/Coding/steam-hardware-watch/runs/2026-06-09/api/steamkit/pics-product-info.json`
- Human-readable detail: `/Users/sean/Coding/steam-hardware-watch/runs/2026-06-09/reports/steamkit-pics-detail.md`

### SteamDB

- SteamDB package pages were challenge-blocked on `2026-06-09`, so no fresh human-readable SteamDB corroboration was collected for the new Machine package changenumbers.
...
- SteamTracking now fast-forwards and records source metadata before scanning. The `2026-06-09` pass used commit `b850a6699d19fc553a8a07be29feb4db88d398e8` (`2026-06-09T01:56:20Z`).
...
- Current ARM64 client manifest baseline in the `2026-06-09` report:
  ...
  - `steam_client_publicbeta_linuxarm64`: version `1780965181`; `bins_hardware_linuxarm64.zip.da8814f3cc3788fd3789ce233dd86f107b2b244a`; size `10039405`.
- ARM64 public-beta client manifest movement appeared versus the `2026-06-05` baseline, including `bins_linuxarm64` and `steam_linuxarm64`. The hardware-specific `bins_hardware_linuxarm64` block did not change.
- The updated SteamTracking checkout did not add a new Machine/Frame reservation package, price, exact availability, or product-publication signal. Content-normalized comparison of the dedicated Frame/Machine reports showed no substantive added or removed focused product strings; the apparent report deltas were line-number churn.
...
- SteamDB's SteamVR depot page remained blocked by challenge pages on `2026-06-09`, so no fresh depot manifest/build comparison was collected.
...
- No Valve public API readiness change appeared on `2026-06-09`.
...
- This remains the cleanest public-readiness baseline. The `2026-06-09` SteamKit/PICS Machine package movement and the corrected SteamTracking guided-tour baseline have not propagated to public `appdetails`, `packagedetails`, packages, package groups, price, or exact timing.

### Customs / Regulatory

- Customs status on `2026-06-09`: partial. ImportInfo automated fetches were blocked, while the existing ImportGenius-backed baseline still showed relevant rows.
...
- Will the `2026-06-09` Machine follow-up changenumber bump precede public Valve API readiness?
